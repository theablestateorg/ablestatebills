...
export default function Customers({ customers, managers, websites }) {
  ...
  managers =
    managers && sortNames
      ? managers.sort((a, b) => a[sortBy] > b[sortBy])
      : managers.sort((a, b) => b[sortBy] > a[sortBy]);
          ...
          <table className="bg-white w-full table-auto p-10 select-none">
            <caption className=" bg-white py-3 outline outline-1 outline-[#e5e7eb] px-3">
              <section className="flex justify-between items-center">
                <h3 className="font-bold text-left">Managers</h3>
                {matches && (
...
export const getServerSideProps = async ({ req, res }) => {
  ...
  if (res) {
    if (
      !person.user ||
      !JSON.parse(person?.user).user.user_metadata.role === "admin"
    ) {
      return {
        redirect: {
          permanent: false,
